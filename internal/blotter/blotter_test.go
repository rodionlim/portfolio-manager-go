...
func createTestTrade() (*Trade, error) {
	return NewTrade("buy", 100, AssetClassEquities, "stock", "AAPL", 150.0, 0.0, time.Now())
}
...
func TestCreateTradeWithInvalidSide(t *testing.T) {
	trade, err := NewTrade("buysell", 100, AssetClassEquities, "stock", "AAPL", 150.0, 0.0, time.Now())
	assert.Error(t, err)
	assert.Nil(t, trade)
}
